...
def draw_text_middle(surface, text, size, color):
    ...
    surface.blit(label, (
        top_left_x + game_width / 2 - (label.get_width() / 2), top_left_y + game_height / 2 - label.get_height() / 2))
...
def show_grid(screen_surface, grid):
    ...
    for eachRow in range(len(grid)):
        pygame.draw.line(screen_surface, (128, 128, 128), (side_x, side_y + eachRow * shape_size),
                         (side_x + game_width, side_y + eachRow * shape_size))
        for eachCol in range(len(grid[eachRow])):
            pygame.draw.line(screen_surface, (128, 128, 128), (side_x + eachCol * shape_size, side_y),
        ...
        for j, column in enumerate(row):
            if column == '0':
                pygame.draw.rect(surface, shape.color,
                                 (sx + j * shape_size, sy + i * shape_size, shape_size, shape_size), 0)
...
def max_score():
    try:
        with open('scores.txt', 'r') as f:
            lines = f.readlines()
            if not lines:
                return 0  # Return a default score if the file is empty
            score = lines[0].strip()
        return score
    except FileNotFoundError:
        return 0  # Return a default score if the file does not exist
...
def create_Grid(screen_surface, grid_scene, score=0, last_score=0):
    ...
    for i in range(len(grid_scene)):
        for j in range(len(grid_scene[i])):
            pygame.draw.rect(screen_surface, grid_scene[i][j],
                             (top_left_x + j * shape_size, top_left_y + i * shape_size, shape_size, shape_size), 0)
